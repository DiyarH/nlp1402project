...
class Statistics():
  ...
  def save_dataset_sizes_stats(self):
    ...
    raw_dataset_size_stats = pd.DataFrame(
      self.raw_dataset_size.items(),
      columns=['category', 'raw dataset size'])
    clean_dataset_size_stats = pd.DataFrame(
      self.clean_dataset_size.items(),
      columns=['category', 'clean dataset size'])
  ...
  def save_n_words_stats(self):
    ...
    n_words_stats = pd.DataFrame(
      self.n_words.items(),
      columns=['category', 'number of words'])
    n_samples = pd.DataFrame(
      self.clean_dataset_size.items(),
      columns=['category', 'number of samples'])
    avg_n_words =\
      n_words_stats['number of words']\
      / n_samples['number of samples'].values
  ...
  def save_n_unique_words_stats(self):
    ...
    n_unique_words_stats = pd.DataFrame(
      self.n_unique_words.items(),
      columns=['category', 'number of unique words'])
    n_unique_words_stats.to_csv(stats_filepath, mode='w', index=False)
  
  def save_most_frequent_words_in_category(self, top_n=10):
    most_frequent_words_directory = os.path.join(config.stats_directory, 'most_frequent_words')
  ...
  def save_TF_IDF_top_words(self, top_n=10):
    tf_idf_top_words_directory = os.path.join(config.stats_directory, 'TF_IDF_top_words')
  ...
  def save_all(self):
    ...
    if len(self.word_frequencies) > 0:
...
class StatisticsExtractor():
  ...
  def extract_RNFs(self, stats_obj: Statistics):
    for category, words in (pbar := tqdm(
      ...
      word_RNFs = tqdm(
          ((word, self.calculate_RNF(word, category, stats_obj)) for word in common_words),
          ncols=config.tqdm_ncols,
      ...
      stats_obj.word_RNFs[category] = {
        word: rnf for word, rnf in word_RNFs
      }


  def calculate_TF_IDF(self, word, category, stats: Statistics):
    tf = stats.wordstats.unique_words[category][word]\
      / stats.n_words[category]
    idf = log(len(stats.wordstats.unique_words)\
